...
{
    if (!_urlArr3) {
        _urlArr3 = [NSMutableArray arrayWithObjects:@"0",@"1",@"2",nil];
    }
    return _urlArr3;
}
...
//    }
    ...
    }else if(self.segement.selectedSegmentIndex ==1)
    {
        ...
        NSString *imageName2 = [NSString stringWithFormat:@"t (%tu)",(indexPath.item +1)];
//        NSLog(@"%@",imageName2);
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *strUrl =[NSString stringWithFormat:@"%@marker/geticonstyle",baseUrl];
    ...
    [manager POST:strUrl  parameters:dic progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        SMAnnoImageModel *model = [SMAnnoImageModel mj_objectWithKeyValues:responseObject[@"result"]];
        [self.annoImageShow  sd_setImageWithURL:[NSURL URLWithString:model.url] placeholderImage:nil options:(SDWebImageLowPriority | SDWebImageRetryFailed ) completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            [self.annoImageShow sizeToFit];
            CDLog(@"%f",self.annoImageShow.width);
        }];
        
        self.lodingImage.hidden = YES;
        self.finalStr = model.url;
